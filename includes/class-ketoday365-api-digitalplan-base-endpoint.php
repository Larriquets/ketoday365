 ...
 */
class Api_Ketoday365_DigitalPlan_Base_Endpoint {
	 ...
	 * @since    0.1.0
	 */
	public function ketoday365_digital_plan_base_api_constructor() {

		register_rest_route( 'keto-endpoint', 'digitalplan-base', array(
			'methods' => 'POST',
			'callback' => array( $this, 'ketoday365_phrase_digital_plan_base' ),
	    ...
	    //// ----  Custom Endpoint - Create Digital Plan BASE ----  ////
		function ketoday365_phrase_digital_plan_base(WP_REST_Request $request) {
			...
			$digital_plan = $this->ketoday365_create_digital_plan_init_base($proteins ,$restrictions );
			...
			function ketoday365_create_digital_plan_init_base( $proteins ,$restrictions ){
				...
				if( ! empty( $posts ) ){
					foreach ( $posts as $p ){ 
						...
						$_recipes[]=  array(
							...
							'meals_checkbox' =>  get_field('meals_checkbox',$p->ID),
							'protein_selection' =>  get_field('protein_selection',$p->ID),
							'lactose_free' =>  get_field('lactose_free',$p->ID),
							'nuts_free' =>  get_field('nuts_free',$p->ID),
							'vegetables' =>  get_field('vegetables',$p->ID),
				...
				foreach($_recipes as $recipe){
					$proteins = $recipe['protein_selection'];
					if(!is_array( $proteins) ){
						   settype($proteins, 'array');
					}
					
					if(in_array("none",$proteins)){
						$json_recipes['veggie'][] = $recipe;
					}else{
						if(!empty($recipe)){
							$json_recipes['proteins'][] = $recipe;	
						}else{
							$json_recipes['proteins'][] = 'empty!';
						}
					}
				}
				// ------ Json Return ------ //
